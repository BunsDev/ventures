...
class Governate extends Component {
  ...
  componentWillMount() {
    ...
    emitter.on(ZAP_RETURNED, this.governateReturned);
  ...
  componentWillUnmount() {
    ...
    emitter.removeListener(ZAP_RETURNED, this.governateReturned);
    emitter.removeListener(GET_BEST_PRICE_RETURNED, this.getBestPriceReturned);
  };


  governateReturned = (txHash) => {
        ...
        <div className={ classes.card }>
          ...
          <Card className={ classes.iHaveContainer }>
            ...
            { (sendAsset && receiveAsset && !(['crvV3', 'crvV4'].includes(receiveAsset.id) && ['crvV1', 'crvV2', 'crvV3'].includes(sendAsset.id)) && !(sendAsset && sendAsset.symbol === 'ETH')) && <Button
              ...
              onClick={ this.onGovernate }
              fullWidth
              >
              <Typography className={ classes.buttonText } variant={ 'h5'} color='secondary'>{ t('Governate.Governate') }</Typography>
            </Button> }
          </Card>
          <div className={ classes.introCenter }>
          </div>
...
export default withNamespaces()(withRouter(withStyles(styles)(Governate)));
